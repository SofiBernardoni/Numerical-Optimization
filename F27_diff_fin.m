...
mean_iter_col = cellfun(@mean, {results_n4.iterations})'; % Converte in colonna
std_iter_col = cellfun(@std, {results_n4.iterations})'; % Converte in colonna
conv_rate_col = (cellfun(@sum, {results_n4.converged}) / (N + 1) * 100)'; % Percentuale di convergenza

% Creazione della tabella
T = table(h_col, mean_iter_col, std_iter_col, conv_rate_col, ...
    'VariableNames', {'h', 'Iterazioni_Medie', 'Dev_Std', 'Perc_Convergenza'});

% Stampa la tabella in console
disp('===== Risultati Convergenza per h =====');
disp(T);
